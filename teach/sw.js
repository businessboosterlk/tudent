// Tudent TEACHER app service worker. Same strategy as the student worker,
// deliberately: two workers with different caching rules is two sets of bugs.
//
// It exists for two reasons. Installability needs a fetch handler before a
// browser will offer "Add to home screen", and the teacher now has real
// offline work to protect: the roll is cached and marks queue in the outbox
// (DECISIONS section 19). A teacher standing in a hall with no signal must be
// able to BOOT the app, not merely keep a tab alive.
//
// Network-first for app code so every
...
const SHELL_CACHE = 'tudent-teacher-shell-v1';
